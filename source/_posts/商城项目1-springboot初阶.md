---
title: 商城项目0-springboot初阶
date: 2018-11-11 23:16:50
categories:
- 商城项目
tags:
- springboot
...
![1525484756737](/pic/1525484756737.png)
...
![1525485888078](/pic/1525485888078.png)
...
新建一个model：
 ...
 ![1525486079389](/pic/1525486079389.png)

使用maven来构建：

![1525486188374](/pic/1525486188374.png)

然后填写项目坐标：

 ![1525486236967](/pic/1525486236967.png)

目录结构：

 ![1525486282929](/pic/1525486282929.png)

项目结构：

 ![1525486435199](/pic/1525486435199.png)
...
这个时候，我们会发现项目中多出了大量的依赖：
 ...
 ![1525486980765](/pic/1525486980765.png)
...
Spring Boot项目通过main函数即可启动，我们需要创建一个启动类：
 ...
 ![1525487293907](/pic/1525487293907.png)
...
我们编写一个controller：
 ...
 ![1525487465325](/pic/1525487465325.png)
...
接下来，我们运行main函数，查看控制台：
 ...
 ![1525487613365](/pic/1525487613365.png)

并且可以看到监听的端口信息：

 ![1525487705460](/pic/1525487705460.png)
...
打开页面访问：http://localhost:8080/hello
 ...
 ![1525487820824](/pic/1525487820824.png)
...
![1525492528558](/pic/1525492528558.png)
...
- 需要注意的是，这里我们并没有指定属性文件的地址，所以我们需要把jdbc.properties名称改为application.properties，这是SpringBoot默认读取的属性文件名：
  ...
  ![1525496325572](/pic/1525496325572.png)
...
![1525496543249](/pic/1525496543249.png)
...
![1525497513206](/pic/1525497513206.png)
...
一切魔力的开始，都是从我们的main函数来的，所以我们再次来看下启动类：
 ...
 ![1525488044650](/pic/1525488044650.png)
...
![1525488226710](/pic/1525488226710.png)
...
![1525488518514](/pic/1525488518514.png)
...
我们跟进源码：
 ...
 ![1525498265579](/pic/1525498265579.png)

并没有看到什么特殊的地方。我们查看注释：

 ![1525498351385](/pic/1525498351385.png)
...
其实在我们的项目中，已经引入了一个依赖：spring-boot-autoconfigure，其中定义了大量自动配置类：
 ...
 ![1525499397690](/pic/1525499397690.png)

还有：

 ![1525499426598](/pic/1525499426598.png)
...
我们来看一个我们熟悉的，例如SpringMVC，查看mvc 的自动配置类：
 ...
 ![1525499859426](/pic/1525499859426.png)

打开WebMvcAutoConfiguration：

![1525500000816](/pic/1525500000816.png)
...
视图解析器：
 ...
 ![1525500405278](/pic/1525500405278.png)

处理器适配器（HandlerAdapter）：

 ![1525500452517](/pic/1525500452517.png)
...
重启服务后测试：
 ...
 ![1525511878440](/pic/1525511878440.png)
...
我们习惯会把静态资源放在`classpath:/static/`目录下。我们创建目录，并且添加一些静态资源：
 ...
 ![1525512196866](/pic/1525512196866.png)

重启项目后测试：

 ![1525512253682](/pic/1525512253682.png)
...
结构如下：
 ...
 ![1525513555179](/pic/1525513555179.png)
...
![1541863789577](/pic/1541863789577.png)

2、输入 lombok 搜索插件， 点install安装，安装完重启idea

![1541863863595](/pic/1541863863595.png)

3.之后就可以直接用注解的方式调日志啦

![1541864000905](/pic/1541864000905.png)
...
当然，在后文中整合了通用mapper的依赖后，他会自动帮我们引好jdbc和mysql的依赖

## 5.3.整合连接池

其实，在刚才引入jdbc启动器的时候，SpringBoot已经自动帮我们引入了一个连接池：

 ![1525514424562](/pic/1525514424562.png)

HikariCP应该是目前速度最快的连接池了，我们看看它与c3p0的对比：

 ![1525516441005](/pic/1525516441005.png)
...
# mapper.xml文件位置,如果没有映射文件，请注释掉，单表的话可以使用通用mapper就不需配置此条
...
![1541920116472](/pic/1541920116472.png)
...
ctrl+F12就可以查看当前类继承了哪些方法

![1541920663407](/pic/1541920663407.png)
...
```java
@Data
@Table(name = "tb_user")//默认会将类名当表名,配置好可以告诉他应该扫哪张表
public class User {
    @Id//告诉他Id是主键
    @KeySql(useGeneratedKeys = true)//告诉他主键自增
    private Long id;
    ...
    @Transient//这个注解标注后不会持久化到数据库，是瞬时变量
    private String note;
...
直接在UserMapper接口上Alt+Enter创建测试类
...
```verilog
2018-11-11 16:09:35.553  INFO 9152 --- [           main] com.alibaba.druid.pool.DruidDataSource   : {dataSource-1} inited
2018-11-11 16:09:36.036 DEBUG 9152 --- [           main] c.l.m.UserMapper.selectByPrimaryKey      : ==>  Preparing: SELECT id,username,password,name,age,sex,birthday,created,updated FROM tb_user WHERE id = ? 
2018-11-11 16:09:36.084 DEBUG 9152 --- [           main] c.l.m.UserMapper.selectByPrimaryKey      : ==> Parameters: 1(Long)
2018-11-11 16:09:36.109 DEBUG 9152 --- [           main] c.l.m.UserMapper.selectByPrimaryKey      : <==      Total: 1
user=User(id=1, username=1, password=1, name=1, age=1, sex=1, birthday=Tue Nov 13 16:06:34 CST 2018, created=Tue Nov 06 16:06:39 CST 2018, updated=Sat Dec 08 16:06:43 CST 2018, note=null)

```

![1541924068583](/pic/1541924068583.png)

在业务里操作表数据，增删数据时用到了事务@Transactional：
...
![1541927951977](/pic/1541927951977.png)

# 6.Thymeleaf快速入门

SpringBoot并不推荐使用jsp，但是支持一些模板引擎技术：

![1525517263421](/pic/1525517263421.png)
...
SpringBoot会自动为Thymeleaf注册一个视图解析器：
 ...
 ![1525522481007](/pic/1525522481007.png)

与解析JSP的InternalViewResolver类似，Thymeleaf也会根据前缀和后缀来确定模板文件的位置：

 ![1525522811359](/pic/1525522811359.png)
...
根据上面的文档介绍，模板默认放在classpath下的templates文件夹，我们新建一个html文件放入其中：
 ...
 ![1525521721279](/pic/1525521721279.png)
...
![1541949061730](/pic/1541949061730.png)

```
注意：
一开始在controller类中用的是@RestController这个注解，这样return的就只是字符串，并不是我们想要的/templates/**.html的格式，此处将其改为@Controller即可。
